...
import { promisify } fro#!/usr/bin/env node
...
const API_SECRET_KEY = process.env.API_SECRET_KEY; // optional, but strongly recommended
...
app.get('/', (req, res) => {
  res.status(200).json({
    status: 'ok',
    message: 'DeepSeek Web Agent API is running',
    endpoints: {
      chat: 'POST /chat (requires Authorization header)',
      health: 'GET /health',
      sessions: 'GET /sessions (admin)'
    }
  });
});
